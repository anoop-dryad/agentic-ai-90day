...
A solid green LED means power is fine and the issue is connectivity. A blinking
red LED means the battery is below 15% and must be replaced. No LED at all means
the sensor has no power — check the wiring harness.

For connectivity issues with a solid green LED, the sensor may have lost its
LoRaWAN connection to the gateway. Sensors reconnect automatically within 15
minutes. If offline longer than 30 minutes, the gateway itself may be down.
