...
@responses.activate
def test_auth_failure_fails_gate():
    """401 → gate fails, marked auth_failed (distinct from not_found)."""
    responses.add(
        responses.GET,
        f"{settings.BACKEND_BASE_URL}/{settings.DEVICE_PATH}/dev-001",
        json={"error": "invalid or missing API key", "code": "UNAUTHORIZED"},
        status=401,
    )
    result = get_device("dev-001")
    assert result.ok is False
    assert result.data == {"auth_failed": True}
